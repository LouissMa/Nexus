...
- `pyproject.toml`: Python package metadata and CLI entry point. Defines 
exus = nexus.cli:main`.
...
- `src/nexus/cli.py`: Command-line interface. Defines commands for memory, goals, proactive review, daily review, briefing, LLM configuration, and RAG memory retrieval.
- `src/nexus/store.py`: JSON persistence layer. Loads and saves local state from `.nexus/state.json` or `NEXUS_HOME/state.json`.
- `src/nexus/service.py`: Main product logic. Handles memory creation/search/retrieval, goals, check-ins, proactive review, daily review/reflection, morning briefing, RAG context construction, and LLM prompt construction.
...
- `tests/test_cli.py`: End-to-end CLI tests and service-level tests for memory, goals, proactive review, daily review, briefing, LLM fallback/config, and RAG retrieval.
